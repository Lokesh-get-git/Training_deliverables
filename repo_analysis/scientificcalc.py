...
from core_utils.config import APP_NAME
from core_utils.logger import get_logger, log_button_click
from core_utils.rate_limiter import MemoryLimiter
...
class Calc():
    ...
    def numberEnter(self, num):
        log_button_click(logger, f"Number {num}")

        current_text = txtDisplay.get()
        proposed = current_text + str(num) if not self.input_value else str(num)

        # MEMORY LIMIT CHECK (INPUT SIZE)
        if not memory_limiter.is_safe_input(proposed):
            logger.warning("Blocked input: exceeds max digit limit")
            return
    ...
    def squared(self):
        log_button_click(logger, "sqrt")

        value = float(txtDisplay.get())
        result = math.sqrt(value)

        # MEMORY LIMIT CHECK (RESULT SIZE)
        if not memory_limiter.is_safe_value(result):
            logger.error("Blocked sqrt result: exceeds safe value range")
            return
